    ...
    >
      <div className="flex gap-4 items-start mx-4 md:ml-[10%] md:mr-0 mt-6">
        <div className="flex flex-col justify-center items-center">
          <div className="w-5 h-5 rounded-full bg-black-red" />
        ...
        <span className="text-4xl">Desenvolvedor Fullstack</span> */}
          ...
          >
            {texts.greetings}
            <span className="text-black-red ml-2 font-bold">Felipe S. Oliveira</span>
          </motion.span>
          <motion.h1
          ...
          >
            {texts.title.map((text, index) => (
              <span
                key={index}
                className={cn("mr-4", text.emphasis && "text-light-red")}
              >
                {text.content}
              </span>
            ))}
          ...
          <div className="flex items-center gap-4 mt-4">
            <Button
              onClick={() => {
                downloadFile("/assets/cv.pdf", "FelieSCV.pdf");
              }}
              data-tooltip-id="my-tooltip"
              data-tooltip-content="CV"
              data-tooltip-place="bottom"
              className="text-sm h-10 px-2 md:px-6 md:h-12 md:text-base w-fit border-light-red"
            >
              {texts.cvButton}
            </Button>
            <Link
              href="https://www.linkedin.com/in/felipe-oliver/"
              target="__blank"
