          ...
          const converted = text
            ...
            .map((word, index) => {
              if (techTexts.highlights.includes(word)) {
                return `<strong key={index} class="text-light-red">
                    ${word}
                  </strong>`;
              }
              return word;
        ...
        <Slider3D
          ...
          sliderItem={(item) => (
            <div className="flex flex-col text-white  transition-all">
              <item.icon className="text-8xl self-center text-light-red" />
              <h3 className="text-xl text-center hover:text-light-red cursor-pointer">
                {item.name}
              </h3>
            </div>
          )}
